...
class Container {
    ...
    constructor(name, description) {
        this._logger = new pip_services3_components_nodex_1.NullLogger();
        this._factories = new DefaultContainerFactory_1.DefaultContainerFactory();
        // Override in child classes
        this._info = new pip_services3_components_nodex_3.ContextInfo(name, description);
    }
    /**
    ...
    initReferences(references) {
        let existingInfo = references.getOneOptional(new pip_services3_commons_nodex_1.Descriptor("*", "context-info", "*", "*", "1.0"));
        if (existingInfo == null) {
            references.put(new pip_services3_commons_nodex_1.Descriptor("pip-services", "context-info", "default", "default", "1.0"), this._info);
        }
        else {
            this._info = existingInfo;
        }
        references.put(new pip_services3_commons_nodex_1.Descriptor("pip-services", "factory", "container", "default", "1.0"), this._factories);
    }
    /**
