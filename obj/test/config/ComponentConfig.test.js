...
suite('ComponentConfig', () => {
    ...
    test('Type', () => {
        ...
        let type = new pip_services3_commons_nodex_2.TypeDescriptor("new name", null);
        componentConfig.type = type;
        assert.equal(componentConfig.type, type);
    });
    test('Descriptor', () => {
        assert.isUndefined(componentConfig.descriptor);
        let descriptor = new pip_services3_commons_nodex_3.Descriptor("group", "type", "id", "default", "version");
    ...
    test('ConfigParams', () => {
        ...
        let config = pip_services3_commons_nodex_1.ConfigParams.fromTuples("config.key", "key", "config.key2", "key2");
        componentConfig.config = config;
        assert.equal(componentConfig.config, config);
    });
    test('From Empty Config', () => {
        let config = pip_services3_commons_nodex_1.ConfigParams.fromTuples();
        try {
    ...
    test('From Wrong Config', () => {
        let config = pip_services3_commons_nodex_1.ConfigParams.fromTuples("descriptor", "descriptor_name", "type", "type", "config.key", "key", "config.key2", "key2");
        try {
            componentConfig = ComponentConfig_1.ComponentConfig.fromConfig(config);
        }
        catch (ex) {
    ...
    test('From Correct Config', () => {
        let descriptor = new pip_services3_commons_nodex_3.Descriptor("group", "type", "id", "name", "version");
        let type = new pip_services3_commons_nodex_2.TypeDescriptor("type", null);
        let config = pip_services3_commons_nodex_1.ConfigParams.fromTuples("descriptor", "group:type:id:name:version", "type", "type", "config.key", "key", "config.key2", "key2");
        componentConfig = ComponentConfig_1.ComponentConfig.fromConfig(config);
